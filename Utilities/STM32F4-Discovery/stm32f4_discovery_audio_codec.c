...
uint16_t *dac_play_ptr;
uint16_t dac_play_size;
uint16_t dac_play_cnt;
...
#ifdef USE_DAC
__IO uint32_t CurrAudioInterface = AUDIO_INTERFACE_DAC;
#endif
...
uint32_t EVAL_AUDIO_Play(uint16_t* pBuffer, uint32_t Size)
{
  ...
  /* Set the total number of data to be played (count in 16-bit) */
